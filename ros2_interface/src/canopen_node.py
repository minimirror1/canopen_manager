...
# common_canopen에서 모듈 임포트
from common_canopen.canopen_motor.motor_manager.motor_controller import MotorController
from common_canopen.canopen_motor.motor_manager.motor_factory import MotorFactory
    ...
    def publish_status(self):
        ...
        try:
            ...
            for motor_info in self.motors_info:
                ...
                try:
                    # 모터 실제 위치 읽기
                    position = self.motor_controller.get_actual_position(node_id)
                    velocity = self.motor_controller.get_actual_velocity(node_id)
                    current = self.motor_controller.get_actual_current(node_id)
                    
                    # 메시지에 추가
                    joint_state.name.append(motor_info['name'])
                    joint_state.position.append(position)
                    joint_state.velocity.append(velocity)
                    joint_state.effort.append(current)
                except Exception as e:
                    self.get_logger().debug(f"모터 ID:{node_id} 정보 읽기 실패: {str(e)}")
